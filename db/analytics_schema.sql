...
SELECT 
    ...
    SUM(gs.duration_seconds)/60 as total_time_minutes,
...
SELECT 
    ...
    AVG(gs.score) as average_score,
    AVG(gs.duration_seconds)/60 as average_duration_minutes,
    COUNT(CASE WHEN gs.completed = true THEN 1 END) as completion_count,
    ROUND(
        (COUNT(CASE WHEN gs.completed = true THEN 1 END)::DECIMAL / COUNT(gs.id)) * 100, 
        2
    ) as completion_rate
FROM jogos j
...
-- 8. Função para calcular streak diário
...
BEGIN
    ...
    IF EXISTS (
        ...
        -- Buscar streak atual
        SELECT current_count, max_count INTO current_streak, max_streak
        FROM user_streaks 
...
-- 9. Trigger para atualizar progresso automaticamente
...
BEGIN
    ...
    DO UPDATE SET
        ...
        average_score = (user_discipline_progress.total_score + COALESCE(NEW.score, 0)) / 
            (user_discipline_progress.games_played + 1.0),
